...
{
    public SneezeEmote(String name, int duration, boolean looping, SoundEvent sound)
    {
    ...
    public void progressAnimation(EntityLivingBase entity, BOBJArmature armature, AnimatorEmoticonsController animator, int tick, float partial)
    {
        ...
        if (tick == Time.toTicks(121) - 1)
        {
            for (int i = 0; i < 10; i ++)
            {
                ...
                entity.world.spawnParticle(EnumParticleTypes.CLOUD, result.x, result.y, result.z, this.rand(0.05F), -0.025F, this.rand(0.05F));
            }
        }
    }
}